            ...
            <div class="col-lg-12 col-md-12 col-sm-12 pt-4 pb-4">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page" @click.prevent>{{$t('all_roles')}}
                        </li>
                    </ol>
                </nav>
            </div>
                ...
                <div class="dashboard_container">
                    <div class="row">
                        <div class="col-12 pt-3">
                            <h2 class="font-weight-bold fun_font text-center">{{$t('all_roles')}}</h2>
                        </div>
                    </div>
                            ...
                            <ul>
                                <li class="list-inline-item">
                                    <button class="btn btn-danger-light"
                                            ...
                                            @click="$router.push({name:'add_roles'})">
                                        <i class="ti-plus"></i>
                                        {{$t('add')}}
                                    </button>
                                </li>
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tr>
                                    ...
                                    <th class="text-left ">{{$t('permissions_count')}}</th>
                                    ...
                                    <td>
                                        <div class="btn-group direction-inverse">
                                            ...
                                            <button class="btn custom_btn btn-success-light"
                                                    v-if="$helper.hasAccessPermission('create-role')"
                                                    @click="$router.push({name:'edit_roles',params:{id:role.id}})">
                                                {{$t('edit')}}
                                            </button>
                                        </div>
                                    </td>
    ...
    export default {
        ...
        methods: {
            ...
            deleteRequest(id) {
                let vm = this;
                vm.$helper.showLoader();
                let dispatch = this.$store.dispatch('moduleRole/removeRole', {
                    ids: [id]
                });
                dispatch.then((response) => {
                    vm.$helper.hideLoader();
                    vm.roles = vm.$helper.deleteMulti([id], vm.roles);
                }).catch((error) => {
                    vm.$helper.handleError(error, vm);
                    vm.$helper.hideLoader();
                });
