            ...
            <div class="col-lg-12 col-md-12 col-sm-12 pt-4 pb-4">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page" @click.prevent>{{$t('all_sliders')}}
                        </li>
                    </ol>
                </nav>
            </div>
                ...
                <div class="dashboard_container">
                    <div class="row">
                        <div class="col-12 pt-3">
                            <h2 class="font-weight-bold fun_font text-center">{{$t('all_sliders')}}</h2>
                        </div>
                    </div>
                            ...
                            <ul>
                                <!--<li class="list-inline-item">-->
                                    <!--<button class="btn btn-danger-light"-->
                                            <!--v-if="$helper.hasAccessPermission('create-slider')"-->
                                            <!--@click="$router.push({name:'add_sliders'})">-->
                                        <!--<i class="ti-plus"></i>-->
                                        <!--{{$t('add')}}-->
                                    <!--</button>-->
                                <!--</li>-->
                                    ...
                                    <td>
                                        <div class="btn-group direction-inverse">
                                            <button class="btn custom_btn btn-primary-light"
                                                    v-if="$helper.hasAccessPermission('delete-slider')"
                                                    @click="deleteSlider(slider.id)">
                                                {{$t('delete')}}
                                            </button>
                                            <button class="btn custom_btn btn-success-light"
                                                    v-if="$helper.hasAccessPermission('update-slider')"
                                                    @click="$router.push({name:'edit_sliders',params:{id:slider.id}})">
                                                {{$t('edit')}}
                                            </button>
                                        </div>
                                    </td>
        ...
        methods: {
            ...
            getAllSliders() {
                ...
                dispatch.then((response) => {
                    vm.sliders = this.$store.getters['moduleSlider/getAllSliders'];
            ...
            deleteRequest(id) {
                let vm = this;
                vm.$helper.showLoader();
                let dispatch = this.$store.dispatch('moduleSlider/removeSlider', {
                    ids: [id]
                });
                dispatch.then((response) => {
                    vm.$helper.hideLoader();
                    vm.sliders = vm.$helper.deleteMulti([id], vm.sliders);
                }).catch((error) => {
                    vm.$helper.handleError(error, vm);
                    vm.$helper.hideLoader();
                });
